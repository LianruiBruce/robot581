...
# Movement Parameters
DRIVE_SPEED = 300             # Motor speed in degrees per second for forward motion
...
TARGET_WALL_DISTANCE_MM = 120 # Target distance from wall during following (15 cm)
...
def handle_collision_recovery_intelligent():
    ...
    global goal_x, goal_y, robot_x, robot_y , last_min_goal_dist
    # (0) 停下来
    left_motor.stop(Stop.BRAKE)
    right_motor.stop(Stop.BRAKE)
    wait(50)
...
def follow_wall_until_hit_point(goal_x, goal_y, hit_x, hit_y,
    ...
    while True:
        ...
        if raw_d > (target_distance_mm + LEFT_CORNER_GAP):
            ...
            turn_in_place_simple(-40, speed=TURN_SPEED)   # 60° 左转，视实际情况可调
            wait(30)
            reset_and_sync_encoders()
            update_odometry()
            ...
            while True:
                ...
                if touch_left.pressed() or touch_right.pressed():
                    ...
                    update_odometry()
                    wait(100)
                    collision_result = handle_collision_recovery_intelligent()
                    if collision_result == "goal":
                        return "goal"
                    elif collision_result == "leave":
                        return "leave"
                    reset_and_sync_encoders()
                    break
